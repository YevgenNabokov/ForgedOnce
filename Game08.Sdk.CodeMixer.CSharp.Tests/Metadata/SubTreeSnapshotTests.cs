...
namespace Game08.Sdk.CodeMixer.CSharp.Tests.Metadata
{
    [TestFixture]
    public class SubTreeSnapshotTests
    {
        ...
        public void CanAnnotate()
        {
            ...
            var subject = new SubTreeSnapshot(codeFile, new SyntaxTreeMappedVisitor<SyntaxTreeMappedVisitorContext>());
            ...
            allAnnotatedNodes.Length.Should().Be(allAnnotatedInRoot.Length);
            annotatedNode.DescendantNodes().ToArray().Length.Should().Be(allAnnotatedNodes.Length - 1);
        }
        ...
        public void CanGetRootWhenUnmodified()
        {
            ...
            var result = subject.ResolveRoots();

            result.Length.Should().Be(1);
            result[0].CurrentPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[1]");
            result[0].OriginalPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[1]");
        }
        ...
        public void CanGetRootsWhenNodeMoved()
        {
            ...
            var classToModify = codeFile.SyntaxTree.GetRoot().DescendantNodes().OfType<ClassDeclarationSyntax>().First(c => c.Identifier.Text == "MyAwesomeClass");
            var fieldToMove = classToModify.DescendantNodes().OfType<FieldDeclarationSyntax>().First(f => f.Declaration.Variables.Count > 0 && f.Declaration.Variables.First().Identifier.Text == "FieldToMove");
            ...
            result[0].CurrentPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[0]/Members[1]");
            result[0].OriginalPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[1]/Members[9]");

            result[1].CurrentPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[1]");
            result[1].OriginalPath.ToString().Should().Be($"{Languages.CSharp}:{fileId}/Members[0]/Members[1]");
        }
